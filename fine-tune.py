...
def train(args):
    ...
    device = torch.device(
        "cuda:{}".format(args.device_id) if torch.cuda.is_available() else "cpu"
    )
    ...
    # Force freeze the CLIP backbone to preserve the "Golden Eyes"
    ...
    Make_dataset_val_post = Makedataset(
    ...
    # ---------------------------------------------------------
    # LOAD CHECKPOINT (VisA Pre-training)
    # ---------------------------------------------------------
    # Replace 'path_to_your_visa_checkpoint.pth' with the actual file path,
    # e.g., './checkpoints/epoch_post_22_1.pth'
    checkpoint_path = (
        args.checkpoint_path
        if hasattr(args, "checkpoint_path")
        else "./my_exps/train_visa/epoch_post_22_2.pth"
    )
    ...
    if os.path.isfile(checkpoint_path):
        ...
        # Your save code uses the key 'MyModel', so we must access that key
        if "MyModel" in checkpoint:
            # strict=True ensures every weight matches perfectly
            msg = MyModel.load_state_dict(checkpoint["MyModel"], strict=True)
            logger.info(f"Checkpoint loaded successfully. Message: {msg}")
        else:
            # Fallback if you saved state_dict directly without the dictionary wrapper
            MyModel.load_state_dict(checkpoint, strict=True)
            logger.info("Checkpoint loaded (direct state_dict).")
    else:
        logger.warning(
    ...
    # --- Strategy: Gentle Prompts, Aggressive Decoder ---
    lr_prompt_fine = args.learning_rate_prompt * 0.1  # Reduce to avoid forgetting VisA
    lr_decoder_fine = (
        args.learning_rate_linear * 5.0
    )  # Boost to learn container texture (ribs/dents)
    ...
    optimizer_stage1 = torch.optim.Adam(
        [
            {"params": lr_group1, "lr": lr_prompt_fine},
            {"params": lr_group2, "lr": 0.01},
            {"params": lr_group3, "lr": args.learning_rate_PFL},
            {"params": lr_group4, "lr": lr_decoder_fine},  # The Pixel Decoder is here
        ],
        lr=learning_rate,
        betas=(0.5, 0.999),
    )
    optimizer_stage2 = torch.optim.Adam(
        [{"params": lr_group5, "lr": args.learning_rate_linear}],
        lr=learning_rate,
        betas=(0.5, 0.999),
    )
    ...
    ema.register()
    post_dataloader, post_obj_list = Make_dataset.mask_dataset(
        name=args.dataset, batchsize=args.batch_size, product_list=None, shuf=True
    )
    if args.dataset == "mvtec":
        ...
        val_dataset_name = "smartshaped"
        val_product_list = val_product_list = ["container"]
    val_post_dataloader, val_obj_list_post = Make_dataset_val_post.mask_dataset(
        name=val_dataset_name, product_list=val_product_list, batchsize=1, shuf=False
    )
    ...
    for epoch_post in range(epochs):
        ...
        for items in train_bar:
            ...
            with torch.no_grad():
                image_features, proj_patch_tokens, patch_tokens = (
                    model_clip.encode_image(image, args.features_list)
                )
            text_embeddings, loss_reg = MyModel.forward_ensemble(
                PFL_TextEncoder,
                image_features,
                patch_tokens,
                cls_name,
            ...
            pro_img, anomaly_maps_list = MyModel(
                text_embeddings, image_features, patch_tokens, stage=stage, mode="train"
            )
            ...
            print(index_prompt)
            pro_img = torch.cat(
                [pro_img[:, index_prompt], pro_img[:, index_prompt + args.prompt_num]],
                dim=1,
            )
            ...
            for num in range(len(anomaly_maps_list)):
                ...
                anomaly_map_select = torch.cat(
                    [
                        anomaly_map[:, index_prompt, :, :],
                        anomaly_map[:, index_prompt + args.prompt_num, :, :],
                    ],
                    dim=1,
                )
            ...
            print(
                f"Segmentation Loss: {loss_seg.item()}, Classification Loss: {loss_class.item()}, Distribution Loss: {loss_dist_reg.item()}, Orthogonal_Loss: {loss_text.item()}"
            )
            del (
                patch_tokens,
                proj_patch_tokens,
        ...
        if (epoch_post + 1) % args.print_freq == 0:
            logger.info(
                "epoch post [{}], loss_seg:{:.4f} loss_cls:{:.4f} loss_dist_reg:{:.4f}  loss_text:{:.4f} ap_raw_pixel:{:.4f}  ap_raw_image:{:.4f}".format(
                    epoch_post + 1,
                    np.mean(loss_seg_list),
                    np.mean(loss_class_list),
                    np.mean(loss_dist_reg_list),
                    np.mean(loss_text_list),
                    ap_raw_pixel,
                    ap_raw_image,
                )
            )
        ...
        if (epoch_post + 1) % args.save_freq == 0:
            ckp_path = os.path.join(
                args.save_path,
                "epoch_post_" + str(epoch_post + 1) + f"_{stage}" + ".pth",
            )
        ...
        if stage == 1:
            if ap_raw_pixel >= ap_max_pixel:
                ...
                logger.info(
                    "epoch post [{}/{}], ap_max_pixel update:{:.4f}".format(
                        epoch_post + 1, epochs, ap_raw_pixel
                    )
                )
                ema.save_check()
            else:
            ...
            if ap_raw_image >= ap_max_image:
                ...
                logger.info(
                    "epoch post [{}/{}], ap_max_image update:{:.4f}".format(
                        epoch_post + 1, epochs, ap_raw_image
                    )
                )
                ema.save_check()
            else:
...
if __name__ == "__main__":
    ...
    # Model
    parser.add_argument(
        "--dataset", type=str, default="smartshaped", help="Training dataset name"
    )
    parser.add_argument("--model", type=str, default="ViT-L-14-336", help="model used")
    parser.add_argument(
        "--pretrained", type=str, default="openai", help="Source of pretrained weight"
    )
    parser.add_argument(
        "--features_list",
        type=int,
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="./my_exps/fine_tune_visa_container",
    ...
    parser.add_argument(
        ...
        help="Original pretrained CLIP path",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="The length of learnable context vectors",
    )  # $P$ in the main text
    parser.add_argument(
        "--prompt_num",
        type=int,
        default=3,
        help="The number of prompts in the prompt bank",
    )  # $B$ in the main text
    parser.add_argument(
        "--prompt_state_len",
        type=int,
        default=5,
        help="The length of learnable state vectors",
    )  # $Q$ in the main text
    parser.add_argument(
        "--learning_rate", type=float, default=0.0001, help="learning rate"
    )
    parser.add_argument(
        "--learning_rate_PFL", type=float, default=0.001, help="learning rate"
    )
    parser.add_argument(
        "--learning_rate_prompt", type=float, default=0.001, help="learning rate"
    )
    parser.add_argument(
        "--learning_rate_linear", type=float, default=0.0001, help="learning rate"
    )
    parser.add_argument(
        "--sample_num",
        type=int,
        default=5,
        help="Number of Monte Carlo samples during validation. Note that during training, we sample only once by default for efficiency",
    )
    ...
    parser.add_argument(
        "--epochs", type=int, default=30, help="Maximum training epochs"
    )
    parser.add_argument(
        "--alpha", type=float, default=0.999, help="Not used in this paper"
    )
